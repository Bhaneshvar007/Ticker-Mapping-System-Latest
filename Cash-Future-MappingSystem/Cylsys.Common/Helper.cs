...
namespace Cylsys.Common
{
    public static class Helper
    {
        ...
        public static DateTime DateFormat3(string vardate)
        {

            if (!string.IsNullOrWhiteSpace(vardate))
            {
                string newstr;
                newstr = vardate.Replace(" 00:00:00", "");
                string[] strdate = newstr.Split('/');
                // string[] strdate = vardate.Split('/');
                if (strdate.Length == 3)
                {
                    vardate = strdate[2] + "-" + strdate[1] + "-" + strdate[0];
                }
            }
            DateTime dt = Convert.ToDateTime(vardate);
            return dt;
        }
